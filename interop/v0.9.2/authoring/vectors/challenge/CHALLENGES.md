...
Nothing in this bundle — and nothing an implementation session may
receive before its freeze — contains a DID, record body, digest,
signature, envelope, winner, or authority-state result for any
challenge case.

These are the previous bundle version's challenge inputs preserved
byte-for-byte; each file's embedded `bundle` member deliberately keeps
its original version marker as provenance. The first, genuinely blind
run of these inputs is completed historical evidence. In the v0.9.2
maintenance pass their rerun is **maintenance confirmation**: an
implementation recomputes its own outputs from this subset alone,
records them, and freezes them at a recorded revision before any
coordinator comparison material is opened. An implementation may see
its own earlier recorded outputs in its own repository; it still must
not receive another implementation's outputs, any comparison result,
or any coordinator expectation before its v0.9.2 freeze.
...
## How an implementation session uses these files
