...
- The CBOR encoding of every relay request and response wrapper an
  ...
  applies to relay messages), with exactly one enumerated exception:
  the optional status `1` publish-response reason code below.

## Enumerated permitted variation — publish status 1

Specification v0.9.2 (Section 12.5) permits a Relay answering a valid
no-change publication to include, or omit, one accurate diagnostic
reason code (`losingRecord` or `duplicate`). The bare and coded forms
are byte-distinct conforming encodings of the same protocol outcome:

- each form is itself byte-deterministic — for a given status and
  reason-code choice there is exactly one conforming encoding;
- a comparison harness compares the received bytes against the
  enumerated conforming set and records which form was chosen;
- the difference between forms is reported as **permitted diagnostic
  variation**, never as a disagreement — and never normalized: one
  form MUST NOT be rewritten, mapped, or summarized into the other;
- any other status and `errorCode` combination — including any
  `errorCode` value outside the Section 15.3 registry, which is the
  complete v1 wire error-code vocabulary — is not variation but a
  malformed response, rejected completely without extracting a status
