//! Interop-interface operations (interop/v0.9.1 INTERFACE.md):
//! `deriveIdentity`, `authorRecord`, `verifyRecord`, `strictEd25519`,
//! `nextTimestamp`, `validateCbor`, `selectCurrent`.
//!
...
//! Contact Document validity, timestamps, selection — is made by the
//! frozen `followee` crate through its public API. The adapter only
//! converts between the neutral interface JSON profile and that API, and
//! enforces the interface input contract (member shapes, hexadecimal and
//! canonical-decimal ranges, authority/signingSeed coherence) before
...
/// Operation failure: either an interface input-contract or adapter
/// infrastructure failure (never a protocol comparison result), or a
/// Followee-level rejection carrying the implementation's symbolic error.
pub enum OpError {
    Infrastructure {
        symbol: &'static str,
        message: String,
...
// ---------------------------------------------------------------------------
// Neutral input readers (interface JSON profile)
// ---------------------------------------------------------------------------
...
/// non-canonical forms and out-of-range values.
...
// ---------------------------------------------------------------------------
// Typed value tree (INTERFACE.md) -> implementation extension types
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Contact Document translation (INTERFACE.md structured contact shape)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Result builders (implementation values -> interface shapes)
// ---------------------------------------------------------------------------
...
/// The `record` member of an accepted `verifyRecord` result.
///
/// INTERFACE.md defines structured shapes for the contact document and
/// extension values but leaves the descriptor sub-shape unspecified; this
/// adapter reuses the shape of the reviewed neutral conformance-runner
/// schema. The comparison layer documents the member-name mapping to the
/// other participant's shape explicitly; the mapped values themselves are
/// compared exactly.
...
/// `deriveIdentity`: every output value is produced by the frozen
/// implementation's public derivation functions.
///
/// The frozen public API exposes no accessor for the raw multihash bytes,
/// so the `multihashHex` member of the interface contract is omitted
/// rather than reconstructed by the adapter; the omission is reported by
/// the comparison layer as a production-surface coverage limitation.
pub fn derive_identity(input: StrictValue) -> Result<Value, OpError> {
    ...
    Ok(json!({
        ...
        "diagnostic": {
            "followeeInteropRust": {
                "multihashHex": "not exposed by the frozen implementation's public API; \
                                 omitted rather than reconstructed in the adapter"
            }
        },
    }))
}

/// `authorRecord`: typed authoring through `RecordBody::encode` and
/// `sign_record`, followed by the implementation's own full verification
/// of the produced envelope as a self-check.
pub fn author_record(input: StrictValue) -> Result<Value, OpError> {
    ...
    // Interface input contract: an incoherent authority/signingSeed
    // pairing is refused, never silently re-keyed.
    ...
    if !coherent {
        return Err(OpError::Infrastructure {
    ...
    // `sign_record` sealed into the envelope; the exact envelope
    // comparison would expose any divergence.
...
/// `verifyRecord`: full verification plus recipient-time classification
/// with the supplied `nowMs`, never the system clock.
...
/// `strictEd25519`: calls the sole production strict verification entry
/// point, `crypto::verify_followee_ed25519` — the same function used by
/// complete record verification.
///
/// The implementation's public contract enforces specification 3.3 rules
/// 1 and 2 (exact 32-byte key, exact 64-byte signature) through its
/// parameter types. A key or signature of any other length therefore
/// cannot form a call at all; translating that documented contract, the
/// adapter reports `valid: false` for such inputs without inventing any
/// cryptographic judgement of its own.
...
/// `nextTimestamp`: calls the implementation's public signer timestamp
/// algorithm with explicit values only.
...
/// `selectCurrent`: verifies every candidate for the explicit target
/// through the implementation's full verification, then invokes its
/// public selection behavior. Candidates that fail verification cannot
/// become `VerifiedRecord` values and are therefore excluded by the
/// implementation's own type discipline, never rewritten.
pub fn select_current_op(input: StrictValue) -> Result<Value, OpError> {
    ...
    let candidates_in = as_array(
        fields.take("candidateEnvelopesHex")?,
    ...
    for (index, item) in candidates_in.into_iter().enumerate() {
        let envelope = decode_hex(
            &as_text(item, "candidateEnvelopesHex entry")?,
            &format!("candidateEnvelopesHex[{index}]"),
        )?;
    ...
    Ok(json!({
        ...
        "diagnostic": {
            "followeeInteropRust": { "candidateErrors": outcomes }
        },
    }))
}

/// `validateCbor`: calls the frozen implementation's public
/// `followee::validate_cbor` — the same deterministic-CBOR validator the
/// record path uses — with the explicit limits supplied by the case. The
/// interface contract bounds `maxDepth` to `0..=8` and `maxMembers` to
/// `0..=256`; out-of-domain values are input-contract violations, never
...
pub fn validate_cbor_op(input: StrictValue) -> Result<Value, OpError> {
    ...
    fields.finish()?;
    if max_depth > 8 {
        return Err(bad_input("maxDepth outside the interface domain 0..=8"));
    }
    if max_members > 256 {
        return Err(bad_input("maxMembers outside the interface domain 0..=256"));
    }
